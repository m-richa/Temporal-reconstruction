        ...
        for batch_idx, data in enumerate(self.data_loader):
            ...
            if batch_idx % self.log_step == 0:
                self.logger.debug('Train Epoch: {} {} Loss: {:.6f}'.format(
                    ...
                    loss.item()))
                self.writer.add_image('input', make_grid(rgb_scf[:, 12:15, :, :].cpu(), nrow=4, normalize=True))
                self.writer.add_image('output', make_grid((output[:, :3, :, :]*500).cpu(), nrow=4, normalize=True))
